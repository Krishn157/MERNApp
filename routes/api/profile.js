...
router.delete('/', auth, async (req, res) => {
  try {
    ...
    //remove user
    await User.findOneAndRemove({ _id: req.user.id }).populate('user', [
      'name',
      'avatar'
    ]);
...
router.put(
  ...
  async (req, res) => {
    ...
    const { school, degree, location, from, fieldofstudy, to } = req.body;
    const newEdu = { school, degree, location, from, fieldofstudy, to };
    try {
      const profile = await Profile.findOne({ user: req.user.id });
